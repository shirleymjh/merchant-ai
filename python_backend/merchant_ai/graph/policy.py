...
class AgentActionRegistry:
    ...
    def __init__(self):
        actions = [
            ...
            AgentAction(id="answer", node="answer_analysis", agent="AnswerAgent", description="compose answer from verified evidence"),
...
class V2AgentPolicy:
    ...
    def decide(self, state: AgentState) -> AgentDecision:
        ...
        selected_action = action_ids[0] if action_ids else "answer"
    ...
    def _candidate_action_ids(self, state: AgentState) -> tuple[List[str], str, bool]:
        ...
        if route and route.route == QuestionRoute.GREETING:
            return ["answer"], "greeting route uses lightweight answer", False
        ...
        if int(state.get("react_round") or 0) >= self.max_main_actions:
            return ["answer"], "main agent action budget exhausted", True
        ...
        if (
            ...
            return ["retrieve_knowledge", "compact_assets", "answer"], "planner requested more semantic knowledge", False
        if (not plan or not plan.intents) and state.get("planner_provider_error"):
            ...
            return ["answer"], "Planner provider failed; answer with structured planning gap", True
        ...
        if reflection and not reflection.passed:
            if (
                ...
                return ["plan_graph", "repair_graph", "answer"], "planner reflection found anchor mismatch; rerun LLM understanding", False
            if (
                reflection.repair_reason == "ANALYSIS_CONTRACT_MISSING"
                and int(state.get("query_graph_plan_attempts") or 0) < self.max_plan_actions
            ):
                return ["plan_graph", "repair_graph", "answer"], "planner reflection found missing analysis evidence contract; rerun LLM understanding", False
            if (
                reflection.repair_reason == "MISSING_REQUIRED_EVIDENCE"
                and not reflection.suggested_knowledge_requests
                and int(state.get("query_graph_plan_attempts") or 0) < self.max_plan_actions
            ):
                return ["plan_graph", "repair_graph", "answer"], "planner reflection found uncovered analysis evidence; rerun LLM understanding", False
            ...
            if (
                ...
                return ["retrieve_knowledge", "repair_graph", "answer"], "planner reflection requested more knowledge", False
            if int(state.get("query_graph_repair_attempts") or 0) < self.max_graph_repair_actions:
                return ["repair_graph", "answer"], "planner reflection found repairable graph issues", False
            return ["answer"], "planner reflection failed and repair budget is exhausted", True
        if not state.get("query_graph_validated"):
        ...
        if validation and not validation.valid:
            if validation.recommended_knowledge_requests and int(state.get("query_graph_retrieve_count") or 0) < self.max_retrieve_actions:
                return ["retrieve_knowledge", "repair_graph", "answer"], "validator requested more semantic knowledge", False
            if validation.repairable and int(state.get("query_graph_repair_attempts") or 0) < self.max_graph_repair_actions:
                return ["repair_graph", "answer"], "validator found repairable graph gaps", False
            return ["answer"], "QueryGraph validation failed and cannot be repaired in budget", True
        ...
        if self.has_graph_repairable_execution_gap(state):
            if int(state.get("query_graph_repair_attempts") or 0) < self.max_graph_repair_actions:
                return ["repair_graph", "answer"], "NodeAgent found graph-repairable plan contract gaps", False
            return ["answer"], "graph-repairable execution gaps remain but repair budget is exhausted", True
        ...
        if self.has_graph_repairable_execution_gap(state):
            if int(state.get("query_graph_repair_attempts") or 0) < self.max_graph_repair_actions:
                return ["repair_graph", "answer"], "evidence verifier found graph-repairable dependency gaps", False
            return ["answer"], "graph-repairable evidence gaps remain but repair budget is exhausted", True
        if not state.get("chat_bi_completed"):
            return ["answer"], "ready to compose BI answer", False
    ...
    def has_graph_repairable_execution_gap(self, state: AgentState) -> bool:
        ...
        return any(gap.code in repairable_codes for gap in run_result.evidence_gaps)
